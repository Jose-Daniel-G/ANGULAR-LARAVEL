...
export class AuthenticationService {
  ...
  saveLocalStorageResponse(resp: any): boolean {
    if (resp.access_token && resp.user) {
      ...
      localStorage.setItem("user", JSON.stringify(resp.user)); // ← ya aquí lo guardas bien
      this.user = resp.user;
  ...
  logout(allDevice: boolean) {
    ...
    return this.http.post(`${environment.URL_SERVICIOS}/logout`,{ allDevice: allDevice },{ headers: headers }
    );
  }
  ...
  getUser() {
    ...
    if (storedUser) {
      this.user = JSON.parse(storedUser);  // Se asegura de que 'user' esté actualizado desde el localStorage
    }
    return this.user;  // Devuelve el usuario almacenado en el servicio
  }
  
  // Register
  register(name: string,email: string,password: string,password_confirmation: string) {
    const data = {name: name,email: email,password: password,password_confirmation: password_confirmation,};
